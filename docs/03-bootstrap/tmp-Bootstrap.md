                ...
                .handler(new ChannelInitializer<SocketChannel>() {
                    ...
                    public void initChannel(SocketChannel ch) {
                        ch.pipeline().addLast(new LoggingHandler(LogLevel.TRACE))
                                .addLast(new ByteEncoder())    // 将用户需要发送的数据类型转换为可以发送的数据类型(ByteBuf)
                                .addLast(new ByteDecoder())    // 将接收的数据类型(ByteBuf)转换为后续用户需要的数据类型
...
public abstract class AbstractBootstrap<B extends AbstractBootstrap<B, C>, C extends Channel> implements Cloneable {
    volatile EventLoopGroup group;                                                                             // 处理事件的线程池
...
## connect触发Bootstrap启动

在调用Bootstrap.connect方法之前,调用Bootstrap的其他方法仅仅是在配置启动信息.调用Bootstrap.connect方法的操作是触发Bootstrap启动的关键步骤.
Bootstrap启动的过程主要是为client-server-channel配置socket参数/配置回调handler/启动连接/向selector注册,后续chanel发生的事件就自动由相应的handler来处理
...
private ChannelFuture doResolveAndConnect(final SocketAddress remoteAddress, final SocketAddress localAddress) {
    final ChannelFuture regFuture = initAndRegister();                                                              // 初始化channel/并将其注册到selector
