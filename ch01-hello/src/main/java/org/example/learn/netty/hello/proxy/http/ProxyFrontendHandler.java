...
public class ProxyFrontendHandler extends SimpleChannelInboundHandler<FullHttpRequest> {
    ...
    protected void channelRead0(ChannelHandlerContext ctx, FullHttpRequest request) {
        ...
        bootstrap.connect(host, port)
                .addListener((ChannelFutureListener) future -> {
                    if (future.isSuccess()) {
                        // 转发请求
                        future.channel().writeAndFlush(adjustRequest(request));
                    } else {
                        ctx.close();
                    }
                });
    ...
    // 移除代理 URI 的 host，改为相对路径
    private FullHttpRequest adjustRequest(FullHttpRequest request) {
        URI uri = URI.create(request.uri());
        String newUri = uri.getRawPath() + (uri.getRawQuery() != null ? "?" + uri.getRawQuery() : "");
